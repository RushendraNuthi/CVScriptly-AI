...
const ProjectsForm: React.FC<Props> = ({ data, updateData }) => {
  ...
  const addProject = () => {
    updateData([...data, { id: `proj${Date.now()}`, name: '', url: '', description: '', tools: [] }]);
  };
      ...
      {data.map((proj, index) => (
        <div key={proj.id} className="p-6 border border-neutral-300 rounded-lg space-y-4 relative">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label="Project Name" id={`name-${index}`} value={proj.name} onChange={(e) => handleChange(index, 'name', e.target.value)} />
            <Input label="Project URL" id={`url-${index}`} value={proj.url} onChange={(e) => handleChange(index, 'url', e.target.value)} />
          </div>
          <Textarea
            label="Description"
