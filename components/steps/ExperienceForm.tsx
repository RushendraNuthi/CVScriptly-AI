        ...
        <div key={exp.id} className="p-6 border border-neutral-300 rounded-lg space-y-4 relative">
          ...
          <div className="relative">
            <div className="flex justify-between items-end mb-1">
              <label htmlFor={`highlights-${index}`} className="block text-sm font-medium text-neutral-700">Highlights / Achievements</label>
              <AIAssistButton
                currentText={exp.highlights.join('\n')}
                onImprove={(text) => handleChange(index, 'highlights', text)}
                context={`Work experience highlights for ${exp.role} at ${exp.company}`}
              />
            </div>
            <textarea
              id={`highlights-${index}`}
