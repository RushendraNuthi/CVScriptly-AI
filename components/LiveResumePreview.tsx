    ...
    const sections: { [key: string]: React.ReactNode } = {
        experience: experience.length > 0 && experience.some(e => e.role) && (
            <section key="experience">
                <h2 className="font-bold border-b-2 pb-1 mb-2" style={{ fontFamily: styling.sectionTitle.family, fontSize: `${styling.sectionTitle.size}pt`, color: styling.sectionTitle.color, borderColor: styling.sectionTitle.color }}>Experience</h2>
                {experience.map(exp => exp.role && (
                    <div key={exp.id} className="mb-4 last:mb-0">
                        <div className="flex justify-between items-baseline flex-wrap">
                            <h3 className="font-bold flex-grow pr-2" style={{ fontFamily: styling.subheading.family, fontSize: `${styling.subheading.size}pt`, color: styling.subheading.color }}>
                                {exp.role}
                                <span className="font-normal" style={{color: styling.font.color}}>{exp.company && `, ${exp.company}`}{exp.location && ` -- ${exp.location}`}</span>
                            </h3>
        ...
        education: education.length > 0 && education.some(e=>e.university) && (
            <section key="education">
                <h2 className="font-bold border-b-2 pb-1 mb-2" style={{ fontFamily: styling.sectionTitle.family, fontSize: `${styling.sectionTitle.size}pt`, color: styling.sectionTitle.color, borderColor: styling.sectionTitle.color }}>Education</h2>
                {education.map(edu => edu.university && (
                    <div key={edu.id} className="mb-2 last:mb-0">
                        <div className="flex justify-between items-baseline flex-wrap">
                            <h3 className="font-bold flex-grow pr-2" style={{ fontFamily: styling.subheading.family, fontSize: `${styling.subheading.size}pt`, color: styling.subheading.color }}>
                              {edu.university}
                              <span className="font-normal" style={{color: styling.font.color}}>{edu.degree && `, ${edu.degree}`}</span>
                            </h3>
        ...
        projects: projects.length > 0 && projects.some(p=>p.name) && (
            <section key="projects">
                <h2 className="font-bold border-b-2 pb-1 mb-2" style={{ fontFamily: styling.sectionTitle.family, fontSize: `${styling.sectionTitle.size}pt`, color: styling.sectionTitle.color, borderColor: styling.sectionTitle.color }}>Projects</h2>
                {projects.map(proj => proj.name && (
                    <div key={proj.id} className="mb-4 last:mb-0">
                        <div className="flex justify-between items-baseline flex-wrap">
                            <h3 className="font-bold flex-grow pr-2" style={{ fontFamily: styling.subheading.family, fontSize: `${styling.subheading.size}pt`, color: styling.subheading.color }}>{proj.name}</h3>
            ...
            <React.Fragment key="customSections">
              {customSections.map(section => section.title && (
                <section key={section.id}>
                  <h2 className="font-bold border-b-2 pb-1 mb-2" style={{ fontFamily: styling.sectionTitle.family, fontSize: `${styling.sectionTitle.size}pt`, color: styling.sectionTitle.color, borderColor: styling.sectionTitle.color }}>{section.title}</h2>
        ...
        skills: skills.length > 0 && skills.some(s => s.skills.some(skill => skill)) && (
            <section key="skills">
                <h2 className="font-bold border-b-2 pb-1 mb-2" style={{ fontFamily: styling.sectionTitle.family, fontSize: `${styling.sectionTitle.size}pt`, color: styling.sectionTitle.color, borderColor: styling.sectionTitle.color }}>Skills</h2>
    ...
    return (
        <div className="bg-white shadow-2xl rounded-lg p-4 sm:p-6 md:p-8 lg:aspect-[210/297] w-full overflow-y-auto" style={bodyStyle}>
            ...
            <div className="space-y-6">
                {summary && (
                    <section>
                        <h2 className="font-bold border-b-2 pb-1 mb-2" style={{ fontFamily: styling.sectionTitle.family, fontSize: `${styling.sectionTitle.size}pt`, color: styling.sectionTitle.color, borderColor: styling.sectionTitle.color }}>Summary</h2>
